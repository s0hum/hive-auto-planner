...
| Lines | Role |
| --- | --- |
| [355-866](index.html#L355-L866) | Constants (`FIELD`, `AUTO_LIMIT`, `LANDMARKS`), the mecanum drivetrain model, the plan model + `seedModel`, the action-library resolvers, Bézier/heading geometry, `solveMove`, `enumerateRoutes` |
| [868-1365](index.html#L868-L1365) | App state (`model`, `sim`, `sel`, `tab`), localStorage persistence + `migrate`, the master `render()`, then the base64 field image (`FIELD_IMG`, ~60 KB on [line 1164](index.html#L1164)) and the SVG field + timeline rendering |
| [1367-1817](index.html#L1367-L1817) | Right rail: step inspector, states pane, actions pane, routes pane |
| [1819-2364](index.html#L1819-L2364) | Java code generation, code pane, pointer/keyboard interaction, bootstrap `render()` |
...
- `action` — an `actionId` pointing into `model.actions`, and nothing else. The name,
  ...
  ([528](index.html#L528)) is the only place that resolves the pointer — it takes the
...
- `parallel` — a `mode` (`parallel` / `deadline` / `race`, the `PARALLEL_MODES` table; the
  id **is** the `Groups` method it exports as) plus `steps[]`. Those steps are the
  arguments to that one Ivy call, one each — so the group is as flat here as it is there,
  and each step is an *arm* that starts when the group does. Arms are `move` / `action` /
  `wait` only: the tree offers no container buttons inside a group, which keeps the two
  tree walks from having to fork a decision that exists on only one of several concurrent
  clocks. `solveParallel` ([774](index.html#L774)) is the only place group timing lives —
  `parallel` takes the slowest arm, `race` the fastest, `deadline` the first — and it picks
  the one driving arm whose end pose becomes the group's. `extraDrive` counts arms that
  also drive (a plan error: one follower, two commands) and `clipped` says the group ends
  before the driving arm's path does, so the pose it threads on is a guess. Arm order is
  meaning, not decoration: `deadline` reads the first one, and the ▲▼ on each row in the
  tree are what set it.

`childLists(s)` ([560](index.html#L560)) is the single answer to "what step lists hang off
this step" — a decision's cases, a group's steps. Every walk that only cares about
...
that thread a pose — `enumerateRoutes`, `generate`'s `flow`, `solveParallel` — do not,
because each has to know what the nesting *means*.
...
`enumerateRoutes` ([796](index.html#L796)) and `generate`'s `flow` ([1904](index.html#L1904)) both walk the step tree with the same explicit frame-stack idiom (`frames` of `{list, i}`, popping exhausted frames, recursing into the chosen case). The first threads a pose and accumulates time to produce every reachable route (capped at `MAX_ROUTES = 96`, setting `truncated`); the second threads a pose and accumulates Java statements. A change to how steps compose — a new `kind`, new nesting — has to land in both or the simulation and the exported code will disagree. A `parallel` step is one frame-stack entry in both: each defers to `solveParallel` for the group's end pose, so a group's steps never enter the frame stack.

A route carries a group as a **single** item of `kind:"parallel"` whose `time` is the
group's elapsed time, not the sum of its arms — which is why anything looking for the
moves a route drives calls `moveItems(route)` ([858](index.html#L858)) rather than
...
`model.robot` is a description of a mecanum base — `motorRpm`, `wheelDia`, `gearRatio`, `massLb`, `eff`, plus the `w`/`h` used for drawing — not a set of motion limits. `drivetrain()` ([419](index.html#L419)) turns it into the numbers the timing needs, and nothing outside that function should read the raw fields to compute speed.

Timing is therefore direction-dependent, which is the point: the four wheels share one speed budget (`holoCap`), so travelling sideways or turning while translating both cost from it. `traverseTime` ([633](index.html#L633)) samples the curve, caps the speed at every sample, then fits a profile under the caps with a forward pass limited by the motor's torque curve and a backward pass limited by braking. That replaced a scalar trapezoid, so a move's time no longer depends only on its length — the heading mode changes it too.
