...
| Lines | Role |
| --- | --- |
| [350-780](index.html#L350-L780) | Constants (`FIELD`, `AUTO_LIMIT`, `LANDMARKS`), the mecanum drivetrain model, the plan model + `seedModel`, the action-library resolvers, Bézier/heading geometry, `solveMove`, `enumerateRoutes` |
| [782-1213](index.html#L782-L1213) | App state (`model`, `sim`, `sel`, `tab`), localStorage persistence + `migrate`, the master `render()`, then the base64 field image (`FIELD_IMG`, ~60 KB on [line 1023](index.html#L1023)) and the SVG field + timeline rendering |
| [1215-1631](index.html#L1215-L1631) | Right rail: step inspector, states pane, actions pane, routes pane |
| [1633-2159](index.html#L1633-L2159) | Java code generation, code pane, pointer/keyboard interaction, bootstrap `render()` |
...
- `action` — an `actionId` pointing into `model.actions`, and nothing else. The name,
  ...
  ([522](index.html#L522)) is the only place that resolves the pointer — it takes the
...
`enumerateRoutes` ([731](index.html#L731)) and `generate`'s `flow` ([1709](index.html#L1709)) both walk the step tree with the same explicit frame-stack idiom (`frames` of `{list, i}`, popping exhausted frames, recursing into the chosen case). The first threads a pose and accumulates time to produce every reachable route (capped at `MAX_ROUTES = 96`, setting `truncated`); the second threads a pose and accumulates Java statements. A change to how steps compose — a new `kind`, new nesting — has to land in both or the simulation and the exported code will disagree.
...
`model.robot` is a description of a mecanum base — `motorRpm`, `wheelDia`, `gearRatio`, `massLb`, `eff`, plus the `w`/`h` used for drawing — not a set of motion limits. `drivetrain()` ([414](index.html#L414)) turns it into the numbers the timing needs, and nothing outside that function should read the raw fields to compute speed.

Timing is therefore direction-dependent, which is the point: the four wheels share one speed budget (`holoCap`), so travelling sideways or turning while translating both cost from it. `traverseTime` ([604](index.html#L604)) samples the curve, caps the speed at every sample, then fits a profile under the caps with a forward pass limited by the motor's torque curve and a backward pass limited by braking. That replaced a scalar trapezoid, so a move's time no longer depends only on its length — the heading mode changes it too.
...
Field inches are the SVG user units directly (`viewBox="0 0 141.5 141.5"`), matching the Pedro Pathing Visualizer: x right, y up, heading in degrees CCW from +x. SVG y grows downward, so every y written into the SVG goes through `fy()` / `poly()` ([1149-1150](index.html#L1149-L1150)) and every pointer y comes back through `fieldXY` ([1974](index.html#L1974)). Model coordinates are always in the Pedro frame; the flip lives only at those two boundaries.
